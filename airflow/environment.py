...
class RoundaboutSegmentedEnv(gym.Env):
    ...
    def _force_turn_from_tip(self, car: Car, options: List[Tuple[str, int]], eps: float = 0.8) -> int:
        ...
        tag, idx = self.rng.choice(options)
        ...
        if isinstance(new_seg, HSeg):
            ...
            car.hx, car.hy = (self.rng.choice([1, -1]), 0)
            car.segment = new_seg
            car.y = new_seg.y
            ...
            car.hx, car.hy = (0, self.rng.choice([1, -1]))
            car.segment = new_seg
            car.x = new_seg.x
    ...
    def reset(self, pairIndex: int, *, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None):
        ...
        dir_val=self.directions[self.rng.integers(0,7)]
        sector_val=self.angles[self.rng.integers(0,2)]
    ...
    def _maybe_transition(self, car: Car, tol: float = 3.0) -> int:
        ...
        if straight_opts and len(turn_opts) == 0:
            ...
            if candidates and self.rng.random() < TRANSITION_CHANCE:
                tag, idx = self.rng.choice(candidates)
                new_seg = self._seg_object((tag, idx))
                car.segment = new_seg
                if isinstance(new_seg, HSeg):
                    car.y = new_seg.y
    ...
    def _seg_object(self, seg_id: Tuple[str, int]):
        tag, idx = seg_id
        return self.hroads[int(idx)] if tag == "H" else self.vroads[int(idx)]
    ...
    def _enter_roundabout(self, car: Car):
        ...
        car.round_dir = 1
        car.roundebout_exit = self.rng.integers(0, 3)
        car._round_progress = 0.0
    ...
    def _draw_roads(self):
        ...
        for h in self.hroads:
            ...
            pygame.draw.rect(self._screen, ASPHALT, pygame.Rect(x1, y - rw//2, x2 - x1, rw))
            pygame.draw.line(self._screen, LANE_MARK, (x1, y), (x2, y), 1)
        # verticals
        for v in self.vroads:
            x = int(v.x * self.ppm)
            y1 = int(min(v.y1, v.y2) * self.ppm)
            y2 = int(max(v.y1, v.y2) * self.ppm)
            pygame.draw.rect(self._screen, ASPHALT, pygame.Rect(x - rw//2, y1, rw, y2 - y1))
            pygame.draw.line(self._screen, LANE_MARK, (x, y1), (x, y2), 1)
    ...
    def _draw_roundabout(self):
        ...
        pygame.draw.circle(self._screen, (230, 230, 230), (cx, cy), R, 2)
    ...
    def _draw_hud(self):
        ...
        if hasattr(self, "beam") and self.beam is not None:
            ...
            lines.append("")
            lines.append(f"Beam x,y: ({self.beam.x:.1f}, {self.beam.y:.1f})")
