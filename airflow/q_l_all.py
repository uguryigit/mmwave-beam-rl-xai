...
from environment import RoundaboutSegmentedEnv
from seed import make_seeds, set_global_seeds, SeedBundle

# ====== TensorBoard (optional) ======
BASE_LOG_DIR = "logs/reinforce/roundabout_q_factorized"
SummaryWriter = None
try:
...
EARLY_STOP_WINDOW         = 50     # compare last 500 vs previous 500
EARLY_STOP_MIN_IMPROVEMENT = 0.02   # absolute delta on rolling mean reward
EARLY_CHECK_EVERY         = 50     # check every N episodes once enough history exists

ANGLE_DEG = [65, 90, 120]
DIR_DEG = [0, 45, 90, 135, 180, 225, 270, 315]


BASE_SAVE_DIR   = "logs/reinforce/roundabout_q_factorized"
...
_env0 = make_env(render=False, seed=SEED0)
...
def run_episode_greedy(q_shared, pairIndex: int, sb: SeedBundle, render: bool = False, max_steps: int = 10_000, ) -> float:
    """Evaluates current Q policy greedily."""
    def q_total(s_base: int) -> np.ndarray:
        return q_shared[s_base]

    e = make_env(render=render, seed=sb.env_seed)
    obs, _ = e.reset(pairIndex=pairIndex, seed=sb.env_seed)
...
def train_one_run(run_id: int) -> dict:
    ...
    for episode in range(EPISODES):
        ...
        while not (terminated or truncated):
            ...
            if terminated or truncated:
                ...
                best_next = float(np.max(q_total(s_next)))
        ...
        if (episode + 1) % EARLY_CHECK_EVERY == 0 and len(ep_rewards) >= 2 * EARLY_STOP_WINDOW:
            ...
            if delta < EARLY_STOP_MIN_IMPROVEMENT:
                ...
                final_eval = run_episode_greedy(Q_shared, pairIndex=run_id, sb=sb, render=False,)
                counts = np.zeros(env.action_space.nvec[0] * env.action_space.nvec[1], dtype=np.int64)
                for i in range(100):
                    obs, info = env.reset(pairIndex=run_id, seed=sb.eval_seed)
                    s_base = base_state_key(obs)
    ...
    for i in eval_seeds:
        obs, info = env.reset(pairIndex=run_id, seed=i.eval_seed)
        s_base = base_state_key(obs)
