...
SEED0 = 42
TOTAL_TIMESTEPS = 1_350_000            # per run
...
def run_one(run_id: int) -> dict:
    ...
    for i in eval_seeds:
        obs, _ = train_env.reset(seed=i.eval_seed)
        ...
        while not (done or trunc):
            ...
            r += rew
    print(f"Tested model {run_id:02d}")
...
def main(run_id, device=0):
    ...
    print("======================\n")
    s, episode_rewards = run_one(run_id)
    all_episode_rewards.append(episode_rewards)
    all_summaries.append(s)
    run_save_dir = os.path.join(LOG_ROOT, f"run_{run_id:02d}")
